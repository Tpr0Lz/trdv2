...
class BuildInstrumentContextTests(unittest.TestCase):
    ...
    def test_macro_context_treats_symbol_as_fred_series(self):
        context = build_instrument_context("DGS10", "macro")
        self.assertIn("FRED series", context)
        self.assertIn("DGS10", context)
        self.assertIn("not as a stock or crypto ticker", context)

    def test_macro_graph_context_skips_yfinance_identity_lookup(self):
        graph = TradingAgentsGraph.__new__(TradingAgentsGraph)
        with patch("tradingagents.graph.trading_graph.resolve_instrument_identity") as mock:
            context = graph.resolve_instrument_context("DGS10", "macro")
