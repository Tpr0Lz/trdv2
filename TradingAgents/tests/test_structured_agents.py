"""Tests for structured-output agents and sentiment report formatting.

The Portfolio Manager has its own coverage in tests/test_memory_log.py
(which exercises the full memory-log → PM injection cycle).  This file
covers the parallel schemas, render functions, and graceful-fallback
behavior we added for the Trader and Research Manager. Sentiment keeps the
same report shape but uses plain LLM invocation so LangGraph can stream tokens.
"""
...
# ---------------------------------------------------------------------------
# Sentiment Analyst: schema, render, streaming-friendly plain generation
# ---------------------------------------------------------------------------
...
def _plain_sentiment_llm(captured: dict, response: str | None = None):
    if response is None:
        response = (
            "**Overall Sentiment:** **Bullish** (Score: 7.5/10)\n"
            "**Confidence:** High\n\n"
            "StockTwits 75% bullish. News constructive. Reddit upbeat."
        )
    llm = MagicMock()
...
class TestSentimentAnalystAgent:
    def test_plain_path_returns_markdown_without_structured_binding(self):
        captured = {}
        response = (
        ...
        llm = _plain_sentiment_llm(captured, response)
    ...
    def test_sentiment_report_also_in_messages(self):
        captured = {}
        analyst = create_sentiment_analyst(_plain_sentiment_llm(captured))
    ...
    def test_prompt_contains_ticker(self):
        captured = {}
        create_sentiment_analyst(_plain_sentiment_llm(captured))(_make_sentiment_state())
