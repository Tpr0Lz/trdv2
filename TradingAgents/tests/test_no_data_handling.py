...
class TestLoadOhlcvNoPoison(unittest.TestCase):
    ...
    def test_rate_limited_history_raises_vendor_rate_limit_and_does_not_cache(self):
        ticker = mock.Mock()
        ticker.history.side_effect = YFRateLimitError()

        with mock.patch.object(stockstats_utils.yf, "Ticker", return_value=ticker):
            with self.assertRaises(VendorRateLimitError):
                stockstats_utils.load_ohlcv("SPY", "2026-01-01")
