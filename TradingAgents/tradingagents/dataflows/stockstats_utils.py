...
def _clean_dataframe(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data["Date"] = _normalize_market_timestamps(data["Date"])
...
def _normalize_market_timestamps(values) -> pd.Series:
    """将带时区的行情时间统一转成无时区时间，便于和分析日期比较。"""
    ...
    # 中文注释：逐值归一化，避免 pd.to_datetime 在混合“有时区/无时区”输入上直接报错。
    normalized = series.map(_normalize_one)
...
def _assert_ohlcv_not_stale(
    ...
    *,
    ...
    if data is None or data.empty:
        return
    requested = _normalize_market_timestamps(pd.Series([curr_date])).iloc[0]
...
def load_ohlcv(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    config = get_config()
    curr_date_dt = _normalize_market_timestamps(pd.Series([curr_date])).iloc[0]
    ...
    end_inclusive = (
        datetime.strptime(end_str, "%Y-%m-%d") + relativedelta(days=1)
    ).strftime("%Y-%m-%d")
    ...
    if data is None:
        ...
        # 中文注释：单 ticker 用 Ticker.history 才能把 429 限流异常继续抛给 yf_retry，
        # 避免 yf.download 吞掉异常后只返回空表，误判成“没有数据”。
        downloaded = yf_retry(
...
def filter_financials_by_date(data: pd.DataFrame, curr_date: str) -> pd.DataFrame:
    ...
    if not curr_date or data.empty:
        return data
    cutoff = _normalize_market_timestamps(pd.Series([curr_date])).iloc[0]
    mask = _normalize_market_timestamps(pd.Series(data.columns)).le(cutoff).to_numpy()
