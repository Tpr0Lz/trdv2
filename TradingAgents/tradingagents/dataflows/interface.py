...
def route_to_vendor(method: str, *args, **kwargs):
    ...
    for vendor in vendor_chain:
        ...
        try:
            ...
            rate_limited_vendors.append(vendor)
            continue
    ...
    if rate_limited_vendors:
        symbol = str(args[0]) if args else "requested symbol"
        vendors = ", ".join(rate_limited_vendors)
        return (
            f"NO_DATA_AVAILABLE: Market data for '{symbol}' is temporarily unavailable "
            f"because configured vendor(s) were rate-limited: {vendors}. Do not estimate "
            "or fabricate values — report that data is unavailable for this symbol."
        )
